...
# Attempts to stitch together original data with the rest of May

def main(path_a, path_b, start_date: datetime):
    print('running')
    print('Period starts:', start_date)

    # Import the tests csv files into a dataframe
    pat_tests_a = pd.read_csv(path_a)
    print('Data import complete')
    print('Dataframe keys:', pat_tests_a.keys())
    unique_patients_a = pat_tests_a['id_patients'].nunique()
    print('unique patients a:', unique_patients_a)
    pat_tests_a = pat_tests_a.drop(columns=['is_pregnant', 'is_smoker',])

    pat_tests_b = pat_tests = pd.read_csv(path_b)
    print('Data import complete')
    print('Dataframe keys:', pat_tests_b.keys())
    unique_patients_b = pat_tests_b['id_patients'].nunique()
    print('unique patients b:', unique_patients_b)
    pat_tests_b = pat_tests_b.drop(columns=['contact_health_worker'])


    # Concatenate and drop duplicates
    pat_tests = pd.concat([pat_tests_a, pat_tests_b], ignore_index=True)
    pat_tests = pat_tests.drop_duplicates(ignore_index=True)

    # Drop countries other than England (remove when looking at whole UK)
    ...
    patients_included_excluded = return_dataframe_of_accepted_patients(pat_tests, start_date, end_date, window,
                                                                       ratio_of_weeks)
    ...
    stat_med, pvalue_med = stats.wilcoxon(england_patients_median['median_before'],
                                          england_patients_median['median_after'])
    ...
    write_df_to_csv(pat_tests_england, 'pat_tests_england_b.csv')
    ...
    # filename = 'pat_tests_england_b.csv'
    #
...
if __name__ == '__main__':
    main(path_a = '/nvme1_mounts/nvme1lv02/coneill/data.csv',
         path_b = '/nvme1_mounts/nvme1lv02/coneill/project_v4/data_05_30_b.csv',
        start_date = datetime.date(2022, 2, 1))
