...
def get_vacc_in_children_uniq_tests_symptoms(src_filename: str,
    ...
    with Session() as s:
       ...
       # Update series names for clarity
        ...
        save_df_to_csv(p_vacc, 'data_without_test_05_30_b.csv', list(p_vacc.keys()))
        ...
        df.merge(p_vacc, src['tests'], dest=v_tests, how='inner', left_on='patient_id', right_on='patient_id',
                 right_fields=['created_at', 'date_taken_specific', 'mechanism', 'result', 'is_rapid_test'], right_suffix='_test')
        # Print summary
        print('At time:', datetime.now(),
              'Unique patients:', len(np.unique(v_tests['id'].data[:])),
        ...
        # Save dataframe to csv
        save_df_to_csv(v_tests, 'data_05_30_b.csv', list(v_tests.keys()))
