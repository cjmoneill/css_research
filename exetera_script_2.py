...
def get_vacc_in_children_uniq_tests_symptoms(src_filename: str,
                                             dst_filename: str,
                                             start_date: datetime,
    ...
    with Session() as s:
        ...
        # filter_1 = d_patients['lsoa11cd'].data[2] == b'E'
        ...
        # Filter assessments data for timeperiod of interest
        ...
        assess_filters_date = (assessments['created_at'].data[:] >= seconds)
        # Apply the filter
        assessments.apply_filter(assess_filters_date)
       ...
       #  df.merge(d_assessments, src['vaccine_doses'], dest=p_vacc, how='outer', left_on='patient_id', right_on='id',
       ...
       #  df.move(p_vacc['date_taken_specific'], p_vacc, 'date_vaccine')
        ...
        # save_df_to_csv(p_vacc, 'data_without_test_05_30_c.csv', list(p_vacc.keys()))
        ...
        df.merge(d_assessments, src['tests'], dest=v_tests, how='left', left_on='patient_id', right_on='patient_id',
                 right_fields=['created_at', 'date_taken_specific', 'mechanism', 'result', 'is_rapid_test'], right_suffix='_test')
        # Print summary
        # print('At time:', datetime.now(),
        #       'Unique patients:', len(np.unique(v_tests['patient_id'].data[:])),
        #       'with number of tests:', len(v_tests['patient_id'].data))
        ...
        # Save dataframe to csv
        save_df_to_csv(v_tests, 'data_05_30_d.csv', list(v_tests.keys()))

if __name__ == "__main__":
    srcfile = '/nvme0_mounts/nvme0lv01/exetera/recent/ds_20220530_full.hdf5'
    dstfile = 'data_v3.hdf5'
    start_date = datetime(2022,2,1)
    get_vacc_in_children_uniq_tests_symptoms(srcfile, dstfile,
                                             start_date,
                                             'tests_vac_symptoms.csv')